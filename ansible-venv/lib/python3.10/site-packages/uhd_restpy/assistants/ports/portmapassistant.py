...
class PortMapAssistant(object):
    ...
    def Connect(
        ...
        self._IxNetwork.info(
            "PortMapAssistant._add_hosts duration: %ssecs" % (time.time() - start)
        )
        start = time.time()
        self._connect_ports(ForceOwnership)
        self._IxNetwork.info(
            "PortMapAssistant._connect_ports duration: %ssecs" % (time.time() - start)
        )
        if IgnoreLinkUp is False:
            start = time.time()
            self._check_link_state(LinkUpTimeout)
            self._IxNetwork.info(
                "PortMapAssistant._check_link_state duration: %ssecs"
                % (time.time() - start)
            )
        else:
        ...
        if len(ip_addresses) > 0:
            ...
            while True:
                ...
                if time.time() - start_time > HostReadyTimeout:
                    raise RuntimeError(
                        "After %s seconds, test port hosts [%s] are not in a ready state"
                        % (HostReadyTimeout, ", ".join(not_ready))
                    )
                time.sleep(3)
    ...
    def _select_vports(self):
        children = [
            {
                ...
                "filters": [{"property": "name", "regex": self._get_name_regex()}],
            },
            {
        ...
        select = Select(
            self._IxNetwork._connection, self._IxNetwork.href, children=children
        )
        return select.go()[0]
    ...
    def _connect_ports(self, ForceOwnership):
        ...
        if ForceOwnership is True:
            self._IxNetwork.Vport.find(Name=self._get_name_regex()).ConnectPorts(
                ForceOwnership
            )
    ...
    def _check_link_state(self, LinkUpTimeout):
        ...
        view = StatViewAssistant(
            self._IxNetwork, "Port Statistics", Timeout=LinkUpTimeout
        )
        view.AddRowFilter("Port Name", StatViewAssistant.REGEX, self._get_name_regex())
        view.CheckCondition(
            "Link State", StatViewAssistant.REGEX, "^Link Up$", Timeout=LinkUpTimeout
        )
